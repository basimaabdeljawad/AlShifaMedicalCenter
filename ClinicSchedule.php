...
if ($method === 'GET') {
    ...
    $sql = "
        ...
        ORDER BY d.full_name ASC, cs.schedule_id ASC
    ";
...
if ($method === 'POST') {
    ...
    if (!$check->fetch()) {
        json_out(false, 'الطبيب غير موجود');
        exit;
    }

    $stmt = db()->prepare("INSERT INTO clinic_schedules (doctor_id, clinic_days, clinic_hours, clinic_location) VALUES (?, ?, ?, ?)");
    $stmt->execute([$doctor_id, $clinic_days, $clinic_hours, $clinic_location]);

    json_out(true, 'تمت إضافة دوام العيادة بنجاح', ['schedule_id' => db()->lastInsertId()]);
    exit;
}
...
if ($method === 'PUT') {
    ...
    $stmt = db()->prepare("UPDATE clinic_schedules SET doctor_id = ?, clinic_days = ?, clinic_hours = ?, clinic_location = ? WHERE schedule_id = ?");
    $stmt->execute([$doctor_id, $clinic_days, $clinic_hours, $clinic_location, $schedule_id]);
